...
クラウド上で面接デモ用に公開する場合は、常時稼働させず、面接後に削除します。

初期デモ構成では、フロントエンドをVercel、バックエンドをLightsailで公開する方針です。

TerraformでLightsailリソースを作成した場合は、デモ後に以下を実行して削除します。

```bash
terraform destroy
```

削除後は、AWSコンソールで以下が残っていないことを確認します。
...
AWS Budgetsは月額 $5 の通知用アラートとして設定済みです。ただし、Budgetsは自動停止ではないため、削除確認は手動で行います。
